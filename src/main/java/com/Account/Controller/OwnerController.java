...
public class OwnerController {
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN') or @ownerSecurity.isSelf(#id, authentication)")
    ...
    @GetMapping("/me")
    public ResponseEntity<Map<String, Object>> getCurrentOwner(@AuthenticationPrincipal UserDetails userDetails) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Map<String, Object>> getOwnerByEmail(@PathVariable String email) {
        User owner = userService.getOwnerByEmail(email);
        Map<String, Object> response = new HashMap<>();
        response.put("id", owner.getId());
        response.put("phone", owner.getPhone());
